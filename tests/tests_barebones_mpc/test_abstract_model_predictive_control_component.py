...
def config_ampcc(setup_mock_config_dict_CartPole):
    return ConfigAMPCC(config=setup_mock_config_dict_CartPole,
                       different_config_instance=None)
...
def config_ampcc_multi(setup_mock_config_dict_CartPole):
    ...
    return ConfigAMPCC(config=setup_mock_config_dict_CartPole,
                       different_config_instance=not_setup_mock_config_dict_CartPole)


class BadMockSampler(MockSampler):

    def _config_pre_init_callback(self, config: Dict, subclass_config: Dict, signature_values_from_config: Dict) -> Dict:
        original_callback_dict = super()._config_pre_init_callback(self, config, subclass_config,
...
class TestAbstractModelPredictiveControlComponent:
    ...
    def test_class_variable(self, config_ampcc_multi):
        ms1 = MockSampler.config_init(model=config_ampcc_multi.model,
                                      config=config_ampcc_multi.config)
        ms2 = MockSampler.config_init(model=config_ampcc_multi.model,
                                      config=config_ampcc_multi.different_config_instance)
